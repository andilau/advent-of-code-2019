...
class Day13(val program: IntArray) : Puzzle {
    ...
    override fun partTwo(): Int {
        val map = program.map(Int::toLong).toLongArray().apply { this[0] = 2 }
    ...
    private fun outputAsSequence(program: IntArray) = sequence {
        val computerWithOutput = CompleteIntCodeComputer(program.map(Int::toLong).toLongArray()).run()
        while (true) {
            try {
